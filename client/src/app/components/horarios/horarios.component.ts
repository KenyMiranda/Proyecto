...
export class HorariosComponent implements OnInit {
  ...
  obtenerNombreMaestro(idGrupo: number): string {
    ...
    if (clase) {
      const maestro = this.arrayMaestros[0].find((m: {id_user:number}) => m.id_user === clase.id_maestro);
  ...
  obtenerNombreMaestro2(idGrupo: number): string {
    ...
    // Si se encuentra la clase, encuentra el maestro correspondiente al id_maestro
    if (clase) {
      const maestro = this.arrayMaestros[0].find((m: {id_user:number}) => m.id_user === clase.id_maestro2);
      return maestro ? `${maestro.first_nameU} ${maestro.last_nameU}` : 'Maestro no encontrado';
    }
